...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    Object(_modules_modals_init_modals__WEBPACK_IMPORTED_MODULE_1__["initModals"])(); // Фокус на поле имени

    var modalToggle = document.querySelector('[data-open-modal]');
    ...
    Array.from(document.querySelectorAll('[data-has-no-js]')).forEach(function (element) {
      return element.classList.remove('has-no-js');
    }); // Функции для кнопок "скрыть"/"показать"

    var getPanelHeight = function getPanelHeight(panel) {
      var panelHeight;
      panelHeight = panel.scrollHeight + 15 + 'px';
      return panelHeight;
    };

    var removeActiveClass = function removeActiveClass(togglesArray, panelsArray) {
    ...
    var updateActiveClass = function updateActiveClass(togglesArray, panelsArray, i) {
    ...
    var setEventListener = function setEventListener(togglesArray, panelsArray) {
      if (togglesArray && togglesArray.length > 0 && panelsArray && panelsArray.length > 0) {
        var _loop = function _loop(i) {
          ...
          togglesArray[i].addEventListener('click', function () {
    ...
    var moreToggles = Array.from(document.querySelectorAll('[data-more-toggle]'));
    var moreContents = Array.from(document.querySelectorAll('[data-more-content]'));
    setEventListener(moreToggles, moreContents); // Аккордеон в footer

    var accordionToggles = Array.from(document.querySelectorAll('[data-accordion-toggle]'));
    var accordionContents = Array.from(document.querySelectorAll('[data-accordion-content]'));
    setEventListener(accordionToggles, accordionContents); // Ресайз высоты панелей аккордеона, текстовых

    var resizeTimeout;

    var resizeThrottler = function resizeThrottler(togglesArray, panelsArray) {
      if (!resizeTimeout) {
        resizeTimeout = setTimeout(function () {
          ...
          for (var i = 0; i < togglesArray.length; i++) {
            ...
            if (togglesArray[i].classList.contains('is-active')) {
              var newPanelHeight = getPanelHeight(panelsArray[i]);
    ...
    var allTogglesArrays = moreToggles.concat(accordionToggles);
    var allContentsArrays = moreContents.concat(accordionContents);
    window.addEventListener('resize', function () {
      return resizeThrottler(allTogglesArrays, allContentsArrays);
    }, false); // Маска для телефона
    ...
    if (phoneInputs && phoneInputs.length > 0) {
      ...
      var onPhoneInput = function onPhoneInput(e) {
        ...
        var firstNumber = input.value[0] !== '+' ? input.value[0] : '';
